...
class PaymentService
{
    public function initialize(Order $order): string
    {
        ...
        $transaction = PaymentTransaction::create(['order_id'=>$order->id,'provider'=>'paystack','reference'=>$reference,'amount'=>$order->total,'currency'=>$order->currency ?? 'NGN','status'=>'pending']);
        $response = Http::withToken($secret)->acceptJson()->post('https://api.paystack.co/transaction/initialize', ['email'=>$order->user?->email,'amount'=>(int) round($order->total*100),'reference'=>$reference,'callback_url'=>route('payment.callback'),'metadata'=>['order_id'=>$order->id]]);
